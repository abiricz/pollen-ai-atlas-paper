...
"""
Distillation Evaluation Script — Reuses Option A's evaluate_classifier.py
==========================================================================

The distilled student model is architecturally IDENTICAL to Option A:
    img → ViT(384) → Linear(384→46) → logits

Therefore we reuse Option A's evaluate_classifier.py EXACTLY.
No adapter, no wrapper — the checkpoint is directly compatible.

This script is a convenience wrapper that:
1. Loads the student checkpoint from Option D's output directory
2. Runs the EXACT same evaluate() and cross-region functions from Option A

Usage:
    python evaluate_distill.py --config ../experiment_config.yaml --experiment distill_all
    python evaluate_distill.py --config ../experiment_config.yaml --experiment distill_all \\
...
# =============================================================================
# EVALUATION (directly uses Option A's model + evaluate)
# =============================================================================
...
def resolve_student_checkpoint(output_dir):
    """Resolve student checkpoint path with robust fallback ordering."""
    candidates = [
        output_dir / "best_model.pth",
        output_dir / "student" / "best_model.pth",
        output_dir / "final_model.pth",
        output_dir / "student" / "final_model.pth",
    ]
...
def run_evaluation(config, experiment_name, test_set=None, device="cuda:0",
    ...
    """Run standard evaluation on TS1/TS2 — identical to Option A."""
    ...
    # Load student model — directly compatible with Option A's load_model()
    ...
    model_path = resolve_student_checkpoint(output_dir)

    if model_path is None:
        print(f"[ERROR] Student model not found under: {output_dir}")
        print("Train first: python train_distill.py --config ... --experiment ...")
        print("Or specify matching --seed/--run_name used during training.")
        sys.exit(1)

    print(f"\n[Model] Loading distilled student from: {model_path}")
    print("[Model] Architecture: ViT → Linear(384→C)  (identical to Option A)")

    # Use Option A's load_model directly — it works because the student
    # has the same architecture (backbone + 384→C head)
    model = load_model(model_path, global_config, device, num_classes)

    # Print distillation metadata if available
    ckpt = torch.load(model_path, map_location="cpu", weights_only=False)
    if "distillation" in ckpt:
        d = ckpt["distillation"]
        print(f"[Distillation] T={d.get('temperature', '?')}, α={d.get('alpha', '?')}, "
              f"teacher_acc={d.get('teacher_val_acc', '?')}")
    if ckpt.get("model_type") == "student_distilled":
        print("[Model] Confirmed: student_distilled checkpoint")
    ...
    # Match Option A's gate exactly: check for explicit mean/std in preprocessing
    if preprocessing_config.get("normalize_mean") and preprocessing_config.get("normalize_std"):
    ...
    for ts_name in test_sets_to_eval:
        ...
        # Confusion matrix plot (match Option A exactly — full matrix, all class names)
        if "confusion_matrix" in results:
            ...
            plot_confusion_matrix(
                ...
                title=f"{exp_config.get('name', experiment_name)} - {ts_name}",
            )

    # Save summary (match Option A's schema EXACTLY — no extra fields)
    summary = {
...
def run_cross_dataset_evaluation(config, experiment_name, source_dataset,
                                  ...
                                  seed=None, run_name=None):
    """Cross-region evaluation — identical to Option A."""
    ...
    if model_path is None:
        print(f"Error: Checkpoint not found under {output_dir}")
        print("Specify matching --seed/--run_name used during training.")
        return None

    # Use Option A's load_model directly
    model = load_model(model_path, global_config, device, num_classes)

    img_size = global_config["img_size"]
    preprocessing_config = exp_config.get("preprocessing", {})

    # Check for experiment-level normalization overrides (match Option A exactly)
    if preprocessing_config.get("normalize_mean") and preprocessing_config.get("normalize_std"):
        mean = preprocessing_config["normalize_mean"]
        std = preprocessing_config["normalize_std"]
    ...
    # Stain normalization (match Option A — cross-region uses training reference)
    stainnorm_func = None
    stainnorm_config = preprocessing_config.get("stainnorm", {})
    if stainnorm_config.get("enabled", False) and HAS_TIATOOLBOX:
        ref_path = stainnorm_config.get("reference_path")
    ...
    for target in all_targets:
        ...
        # Match Option A exactly: for self-evaluation, all species are overlapping
        if is_self:
            overlapping = set(s.lower() for s in species_to_int.keys())
            overlap_count = "all"
        else:
        ...
        print(f"\n{'=' * 60}")
        print(f"Cross-region: {label}")
        print(f"Overlapping species: {overlap_count}")
        print(f"{'=' * 60}")
        ...
        cross_results[result_key] = {
            ...
            "overlap_taxa_count": overlap_count,
            "all": results.get("all", results),
            "overlap": results.get("overlap", results),
            "overlap_species": sorted(overlapping) if not is_self else [],
        }
    ...
    # Save summary (match Option A schema — no extra "type" field)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate distilled student classifier (Option D)")
    ...
    if args.source_dataset and args.target_datasets:
        ...
        run_evaluation(
            config, args.experiment, args.test_set, args.device,
            seed=args.seed, run_name=args.run_name,
        )
